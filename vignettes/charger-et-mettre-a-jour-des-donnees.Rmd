...
Ce tutorial explique comment utiliser les fonctions `add_or_update_*` qui
permettent, comme leur nom l'indique, de mettre à jour un objet existant et s'il
n'existe pas de le créer.

Un objet est identifié par son titre (et son parent pour les fichiers de données
et les fichiers joints), cela a plusieurs conséquences :
...
données**.
2. il n'est pas possible de modifier le titre d'un objet (pour cela vous pouvez
utiliser `update_datafile()`).

Toutes ces fonctions vont chercher un objet **du même type** avec **le même
titre** puis :
